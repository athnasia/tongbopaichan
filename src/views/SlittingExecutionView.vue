...
const allRecords = ref<ProductionRecord[]>([])
...
onMounted(async () => {
  const res = await slittingApi.getRecords()
  allRecords.value = res.data as ProductionRecord[]
})
</script>
...
<template>
  <div class="space-y-4">
    ...
    <el-card shadow="never">
      ...
      <el-empty v-if="filteredRecords.length === 0" description="暂无分切生产记录" />
      <template v-else>
